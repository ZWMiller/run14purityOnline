...
//  StThreeVectorF pVertex = ev->eventSummary().primaryVertexPosition();
  ...
  for(int i=0;i<nTrigger;i++) {
    if(ev->triggerIdCollection().nominal().isTrigger(450201)
    || ev->triggerIdCollection().nominal().isTrigger(450211)
    || ev->triggerIdCollection().nominal().isTrigger(450202)
    || ev->triggerIdCollection().nominal().isTrigger(450212)
    || ev->triggerIdCollection().nominal().isTrigger(450203)
    || ev->triggerIdCollection().nominal().isTrigger(450213)
			){
      isTrg = kTRUE;
      break;
    }
  ...
  */
  ...
  if(!isTrg) {
    LOG_DEBUG << "StPicoCut::passEvent trigger not fired " << endm;
    return kFALSE;
  }
